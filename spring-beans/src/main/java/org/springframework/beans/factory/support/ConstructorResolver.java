...
class ConstructorResolver {
	...
	public BeanWrapper autowireConstructor(String beanName, RootBeanDefinition mbd,
		...
		if (constructorToUse == null || argsToUse == null) {
			...
			if (candidates == null) {
				...
				try {
					//isNonPublicAccessAllowed 允许非公共访问
					// 获取所有public或非public构造方法
	...
	private Object instantiate(
		...
		try {
			...
			if (System.getSecurityManager() != null) {
				...
				// 策略接口实例化，CglibSubclassingInstantiationStrategy 中默认实现，使用cglib代理
	...
	public BeanWrapper instantiateUsingFactoryMethod(
		...
		if (explicitArgs != null) {
			...
			synchronized (mbd.constructorArgumentLock) {
				...
				if (factoryMethodToUse != null && mbd.constructorArgumentsResolved) {
					...
					argsToUse = mbd.resolvedConstructorArguments;
					if (argsToUse == null) {
						argsToResolve = mbd.preparedConstructorArguments;//相当于二级缓存
					}
				}
			}
			...
			// 缓存中的值可能是原始值也有可能是最终值
			if (argsToResolve != null) {
				// 可能是从preparedConstructorArguments取出的，还需要解析成最终值
		...
		if (factoryMethodToUse == null || argsToUse == null) {
			...
			// bean中只有一个静态的 且无参的 方法。并且没有设置构造函数参数值
			...
			// 构造函数参数个数
			int minNrOfArgs;
			// 传入则直接取，没有传则从RootBeanDefinition中解析
